...
if (args[1].value) {
  ...
  let xhw0 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.${instanceIndex}.X_HW_VLAN`, {value: Date.now() - cacheAge});
  let xhw1 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${instanceIndex}.WANIPConnection.1.X_HW_VLAN`, {value: Date.now() - cacheAge});
  let cmcc0 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.${instanceIndex}.X_CMCC_VLANIDMark`, {value: Date.now() - cacheAge});
  let cmcc1 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${instanceIndex}.WANIPConnection.1.X_CMCC_VLANIDMark`, {value: Date.now() - cacheAge});
  if (xct.size) {
    m = xct.value[0];
  } else if (xhw0.size) {
    m = xhw0.value[0];
