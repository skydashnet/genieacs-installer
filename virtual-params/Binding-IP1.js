...
if (args[1].value) {
  ...
  let xct = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${instanceIndex}.WANIPConnection.1.X_CT-COM_LanInterface`, {value: Date.now() - cacheAge});
  let cmcc0 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.${instanceIndex}.X_CMCC_LanInterface`, {value: Date.now() - cacheAge});
  let cmcc1 = declare(`InternetGatewayDevice.WANDevice.1.WANConnectionDevice.${instanceIndex}.WANIPConnection.1.X_CMCC_LanInterface`, {value: Date.now() - cacheAge});
  if (xct.size) {
    m = xct.value[0];
  } else if (cmcc0.size) {
    m = cmcc0.value[0];
